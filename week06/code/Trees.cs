public static class Trees
{
    ...
    private static void InsertMiddle(int[] sortedNumbers, int first, int last, BinarySearchTree bst)
    {
        ...
        // Validate that the array is not null and that the binary search tree is also present
        if (sortedNumbers == null || bst == null || first > last)
        {
            return;
        }
        ...
        // Insert the middle element into the binary tree
        bst.Insert(sortedNumbers[middle]);

        // Recursively insert the elements from the left and right half
        InsertMiddle(sortedNumbers, first, middle - 1, bst);  // Insert left half
